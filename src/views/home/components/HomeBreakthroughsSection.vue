...
const fetchBreakthroughItems = async () => {
  ...
  const itemsData = [];
  const texts = [ '光学声子软化新理论：突破半导体器件功耗瓶颈新希望', '浙江大学PEDL团队五项重要成果在功率器件顶级会议ISPSD发表', '镓仁半导体成功制备VB法100毫米(010)面氧化镓单晶衬底', '高整流、超低漏P-Si/N-ALN异质结PN二极管', '众专家学者共话硅、碳化硅器件及其他高压功率器件进展', '高整流、超低漏P-Si/N-ALN异质结PN二极管 (重复示例)', '众专家学者共话硅、碳化硅器件及其他高压功率器件进展 (重复示例)', '第八条成果，用于测试分页', '第九条成果，也用于测试分页' ];
